...
type fakeScanErrorsUC struct {
	called int
	scanID uuid.UUID
}

func (f *fakeScanErrorsUC) Execute(_ context.Context, id uuid.UUID) error {
	f.called++
	f.scanID = id
	return nil
}

func TestGetScanErrorsCmd(t *testing.T) {
	...
	require.NoError(t, cmdtest.Execute(t, root.Command, "scan", "errors", scanID.String(), "-p", projectID.String()))
