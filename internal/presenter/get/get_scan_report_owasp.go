...
func NewGetScanReportOwaspCmd(uc UseCaseGetScanReportOwasp) CmdGetScanReportOwasp {
	cmd := &cobra.Command{
		Use:   "owasp <scan-id>",
		Short: "Get scan report in OWASP format",
		Long:  `Download the scan report in OWASP format for the given scan id. Project id comes from context or parent -p. Output path via -o; use -f to overwrite.`,
		Example: `  aictl get scan report owasp <scan-id> -o ./out.xml
  aictl get scan report owasp <scan-id> -o ./out.xml -f`,
		Args: cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			ctx := cmd.Context()

			if err := uc.Execute(ctx, scanId, report.Owasp, outPath, includeComments, includeDFD, includeGlossary, l10n, report.EmptyFilters()); err != nil {
