...
func (a *ClientAI61) GetReport(ctx context.Context, projectId, scanResultId, templateId uuid.UUID, includeComments, includeDFD, includeGlossary bool, l10n string, filters report.Filters) (io.ReadCloser, error) {
	...
	if filters.Apply {
		apiFilters, err := toUserReportFiltersModel61(filters)
		if err != nil {
			return nil, err
		}
...
func toUserReportFiltersModel61(filters report.Filters) (v6_1.UserReportFiltersModel, error) {
	types := filters.Types
	if types == nil {
		types = []string{}
	}
	languages, err := mapProgrammingLanguages61(filters.Languages)
	if err != nil {
		return v6_1.UserReportFiltersModel{}, err
	}
	modules, err := mapScanModules61(filters.ScanModules)
	if err != nil {
		return v6_1.UserReportFiltersModel{}, err
	}

	return v6_1.UserReportFiltersModel{
...
func mapProgrammingLanguages61(langs []string) ([]v6_1.ProgrammingLanguageGroup, error) {
	out := make([]v6_1.ProgrammingLanguageGroup, 0, len(langs))
	for _, lang := range langs {
		pl := v6_1.ProgrammingLanguageGroup(lang)
		if !pl.Valid() {
...
func mapScanModules61(modules []string) ([]v6_1.ScanModuleType, error) {
	out := make([]v6_1.ScanModuleType, 0, len(modules))
	for _, m := range modules {
		sm := v6_1.ScanModuleType(m)
		if !sm.Valid() {
...
func isScanStopFallbackError(err error) bool {
	return apperror.IsApiErrorCode(err, string(v6_1.ApiErrorTypeQUEUEITEMNOTFOUND)) ||
		apperror.IsApiErrorCode(err, string(v6_1.ApiErrorTypeQUEUEITEMALREADYASSIGNEDTOAGENT)) ||
		apperror.IsApiErrorCode(err, string(v6_1.ApiErrorTypeSCANNOTFOUND))
}
...
func (a *ClientAI61) CheckLicense(ctx context.Context) (*domainlicense.License, error) {
...
func (a *ClientAI61) GetScanIssues(ctx context.Context, projectId, scanResultId uuid.UUID) ([]statistic.Issue, error) {
	...
	return mapScanIssues61(*response.JSON200), nil
}

func mapScanIssues61(issues []v6_1.VulnerabilityModel) []statistic.Issue {
