...
type AI interface {
	...
	GetScanQueue(ctx context.Context) ([]domainqueue.Entry, error)
}
...
// Execute lists scan queue. projectFilter is opt-in via -p only; ctx project is ignored.
func (u *UseCase) Execute(ctx context.Context, projectFilter *uuid.UUID) error {
	...
	entries, err := u.aiAdapter.GetScanQueue(ctx)
	if err != nil {
		return fmt.Errorf("get scan queue: %w", err)
	}
