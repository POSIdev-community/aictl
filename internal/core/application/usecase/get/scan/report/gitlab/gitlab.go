...
func (u *UseCase) Execute(ctx context.Context, scanId uuid.UUID, fullDestPath string, includeComments, includeDFD, includeGlossary bool, l10n string) error {
	...
	u.cliAdapter.ShowTextf(ctx, "getting gitlab scan report, scan-id '%v'", scanId.String())

	templateId, err := u.aiAdapter.GetTemplateId(ctx, report.GitlabReportType)
	if err != nil {
		return err
	}
	...
	u.cliAdapter.ShowText(ctx, "gitlab scan report got")
