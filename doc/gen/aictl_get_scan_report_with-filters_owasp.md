## aictl get scan report with-filters owasp

Get scan report in OWASP format

### Synopsis

Download the scan report in OWASP format for the given scan id. Project id comes from context or parent -p. Output path via -o; use -f to overwrite.

```
aictl get scan report with-filters owasp <scan-id> [flags]
```

### Examples

```
  aictl get scan report with-filters owasp <scan-id> -o ./out.xml --level-high
  aictl get scan report with-filters owasp <scan-id> -o ./out.xml -f --level-high
```
