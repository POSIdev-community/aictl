## aictl get scan report with-filters xml

Get scan report in XML format

### Synopsis

Download the scan report in XML format for the given scan id. Project id comes from context or parent -p. Output path via -o; use -f to overwrite.

```
aictl get scan report with-filters xml <scan-id> [flags]
```

### Examples

```
  aictl get scan report with-filters xml <scan-id> -o ./out.xml --level-high
  aictl get scan report with-filters xml <scan-id> -o ./out.xml -f --level-high
```
