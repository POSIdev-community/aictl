## aictl get scan report with-filters autocheck

Get scan report in Autocheck format

### Synopsis

Download the scan report in Autocheck format for the given scan id. Project id comes from context or parent -p. Output path via -o; use -f to overwrite.

```
aictl get scan report with-filters autocheck <scan-id> [flags]
```

### Examples

```
  aictl get scan report with-filters autocheck <scan-id> -o ./out.autocheck --level-high
  aictl get scan report with-filters autocheck <scan-id> -o ./out.autocheck -f --level-high
```

### Options

```
  -h, --help   help for autocheck
```
