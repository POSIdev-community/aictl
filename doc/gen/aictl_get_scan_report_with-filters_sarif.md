## aictl get scan report with-filters sarif

Get scan report in SARIF format

### Synopsis

Download the scan report in SARIF format for the given scan id. Project id comes from context or parent -p. Output path via -o; use -f to overwrite.

```
aictl get scan report with-filters sarif <scan-id> [flags]
```

### Examples

```
  aictl get scan report with-filters sarif <scan-id> -o ./out.sarif --level-high
  aictl get scan report with-filters sarif <scan-id> -o ./out.sarif -f --level-high
```
