## aictl get scan report with-filters owaspm

Get scan report in OWASP Mobile format

### Synopsis

Download the scan report in OWASP Mobile format for the given scan id. Project id comes from context or parent -p. Output path via -o; use -f to overwrite.

```
aictl get scan report with-filters owaspm <scan-id> [flags]
```

### Examples

```
  aictl get scan report with-filters owaspm <scan-id> -o ./out.owaspm --level-high
  aictl get scan report with-filters owaspm <scan-id> -o ./out.owaspm -f --level-high
```

### Options

```
  -h, --help   help for owaspm
```
