...
Get scan report in plain text format

### Synopsis

Download the scan report in plain text format for the given scan id. Project id comes from context or parent -p. Output path via -o; use -f to overwrite.
...
```
  aictl get scan report plain <scan-id> -o ./out.txt
