## aictl get scan report with-filters plain

Get scan report in plain HTML format

### Synopsis

Download the scan report in plain HTML format for the given scan id. Project id comes from context or parent -p. Output path via -o; use -f to overwrite.

```
aictl get scan report with-filters plain <scan-id> [flags]
```

### Examples

```
  aictl get scan report with-filters plain <scan-id> -o ./out.html --level-high
  aictl get scan report with-filters plain <scan-id> -o ./out.html -f --level-high
```
