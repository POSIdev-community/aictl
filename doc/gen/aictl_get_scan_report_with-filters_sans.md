## aictl get scan report with-filters sans

Get scan report in SANS format

### Synopsis

Download the scan report in SANS format for the given scan id. Project id comes from context or parent -p. Output path via -o; use -f to overwrite.

```
aictl get scan report with-filters sans <scan-id> [flags]
```

### Examples

```
  aictl get scan report with-filters sans <scan-id> -o ./out.sans --level-high
  aictl get scan report with-filters sans <scan-id> -o ./out.sans -f --level-high
```

### Options

```
  -h, --help   help for sans
```
