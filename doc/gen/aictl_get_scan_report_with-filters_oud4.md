## aictl get scan report with-filters oud4

Get scan report in OUD4 format

### Synopsis

Download the scan report in OUD4 format for the given scan id. Project id comes from context or parent -p. Output path via -o; use -f to overwrite.

```
aictl get scan report with-filters oud4 <scan-id> [flags]
```

### Examples

```
  aictl get scan report with-filters oud4 <scan-id> -o ./out.oud4 --level-high
  aictl get scan report with-filters oud4 <scan-id> -o ./out.oud4 -f --level-high
```

### Options

```
  -h, --help   help for oud4
```
